...
let Best = ({ pro }) => {
    ...
    let { CatFunction2, cat, increaseHeight, showClear, setScroll } = useContext(CatContext)
    ...
    return (
        <div className="best">
            ...
            <div style={{ transform: `translateX(${slideIndex * -240}px)` }} className='best-container'>
                ...
                {pro.map(item =>
                    <Link to={`/single/${item.id}`} key={item.id} className='best-box' onClick={() => setScroll(true)}>
                        <img src={item.image} />
                        <h6>{item.title}</h6>
